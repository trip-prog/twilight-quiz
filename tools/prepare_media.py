...
arguments = [arg for arg in sys.argv[1:] if arg not in ['--extra', '--force']]
questions = json.loads((root/('extra-questions.json' if extra else 'questions.json')).read_text(encoding='utf-8'))
...
if arguments: questions=[q for q in questions if q['film'] in list(map(int,arguments))]
...
def render(q):
    ...
    for path, start, end in ranges:
        ...
        chapter = source['chapters'][q['film']-1]['start_time']
        c = next((c for c in candidates.values() if c['film'] == q['film'] and chapter+c['start'] <= start and end <= chapter+c['end']), None)
        candidate = root/'.work/candidates'/f"{c['id']}.mp4" if c else None
        ...
        command = ['ffmpeg','-hide_banner','-loglevel','error','-y',*inputs,'-t',str(duration),'-vf',"scale=w='min(960,iw)':h='min(540,ih)':force_original_aspect_ratio=decrease:force_divisible_by=2,setsar=1",'-r','25','-c:v','libx264','-preset','fast','-crf','27' if extra else '24','-pix_fmt','yuv420p','-threads','2','-af',f'afade=t=in:d=0.03,afade=t=out:st={duration-.03:.3f}:d=0.03','-c:a','aac','-b:a','80k' if extra else '96k','-ar','48000','-movflags','+faststart',str(temporary)]
